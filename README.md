...
命令范围为 `-10000~10000`。四个DRV8870输入现在全部使用硬件PWM，并采用驱动/制动慢衰减：

- 正转：IN1 固定 100%，IN2 输出 `100%-命令占空比`。
- 反转：IN2 固定 100%，IN1 输出 `100%-命令占空比`。
- 停止：IN1、IN2占空比均为0，进入滑行/低功耗状态。

运行时在 `1/0` 或 `0/1` 驱动与 `1/1` 低侧制动之间切换，使关断阶段电流慢衰减。命令为 30% 时对应约 30% 驱动和 70% 制动；命令为 0 时单独进入 `0/0` 停止状态。
...
历史驱动/滑行模式下，20kHz 时四路输入的万用表平均电压正确但电机无动作，1kHz 能够正常驱动。该失败现已定位为 PWM 关断阶段使用 `0/0` 快速衰减，并非 DRV8870 不支持高频。
...
最终实物复测中，改用非阻塞状态机后OLED阶段、PB2心跳及左右电机正反转均恢复正常，说明此前停留在`LEFT FWD`的直接原因是阻塞式测试流程没有继续推进，而不是DRV8870、PCB控制网络或电机损坏。

改为驱动/制动慢衰减后，30% 占空比架空实测结果为：10kHz 左 373/右 369mm/s，20kHz 左 370/右 368mm/s；两次均持续运行、无异响、转动平滑且驱动模块无明显温升。因此当前软件采用 20kHz。第二版原理图确认R13~R16是四颗独立的10k下拉电阻，不是串联电阻，也没有与输入组成RC滤波。

R11 已从 12kΩ 更换为 4.7kΩ、1%，R12 保持 10kΩ，VREF 实测 2.23V。配合模块 0.1Ω 采样电阻，对应约 2.23A 限流。20kHz、30% 占空比架空复测：正转左 360/右 365mm/s，反转左 359/右 363mm/s；均连续转动、无异响且模块温度正常。
