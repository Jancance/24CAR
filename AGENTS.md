...
- 四路控制均为硬件 PWM：
  ...
  - 右 IN2：`PB18/TIMA1_C1`
- 实物方向：
  - 左轮前进：PA12/IN1 输出 PWM，PA8/IN2 为 0。
  - 左轮后退：PA12/IN1 为 0，PA8/IN2 输出 PWM。
  - 右轮前进：PA13/IN1 为 0，PB18/IN2 输出 PWM。
  - 右轮后退：PA13/IN1 输出 PWM，PB18/IN2 为 0。
- 方向宏保持：左轮不反相，右轮反相，即 `CAR_MOTOR_LEFT_REVERSED=0`、`CAR_MOTOR_RIGHT_REVERSED=1`。
- `motor_set()` 的正值统一表示车辆前进方向，负值表示后退。
- 当前实物可靠 PWM 频率为 1 kHz。5 kHz 时电机会响但不启动；20 kHz 平均电压看似正常但电机不转。未完成示波器和电流检查前不要提高频率。
...
- 最终左轮PI为Kp20、Ki80、Kd0，积分输出限4000 PWM、启动斜坡500 mm/s²；最终右轮PI为Kp20、Ki40、Kd0，积分输出限4500 PWM（积分状态±112.5）、启动斜坡1000 mm/s²。总输出上限均为8000 PWM。
...
- `main.c` 只调用一次 `ALL_Init()`，随后在主循环调用 `speed_control_loop()`，等待后续正式任务模块接入。
- `ALL_Init()` 初始化速度控制器，但上电不设置运行状态，四路PWM保持为0，因此不会自行驱动车轮。
- UART3/Serial通用模块仍保留供后续无线调参或遥测使用，但当前主循环不再运行速度测试命令解析、OLED测试页面或按键自动行驶流程。
