...
- 当前调试蓝牙串口参数为 `115200, 8N1`。蓝牙模块连接 UART3：模块TX接PA25、模块RX接PA26，并与小车共地；USB-TTL与蓝牙同一时刻只能接一个。
...
- 当前速度环为左轮 `Kp=20, Ki=80, Kd=0`、右轮 `Kp=20, Ki=50, Kd=0`，控制周期20 ms；蓝牙已验证100/200/300 mm/s和左右轮轻微加载恢复，右 Ki=50 仍需地面长距离验证。左编码器前进方向需配置 `CAR_ENCODER_LEFT_REVERSED=1`。
- `speed_control.c/.h` 保留为正式双轮速度闭环：20 ms为最小调度间隔，但编码器速度和PID dt必须使用实际经过时间；上电默认停止，反向反馈或超速会同时停两轮。
- 旧 1kHz 驱动/滑行速度环参数为左 Kp20/Ki80、右 Kp20/Ki40；当前20kHz驱动/制动使用左Kp20/Ki80、右Kp20/Ki50，总输出上限均为8000 PWM，当前左右启动斜坡均为1000mm/s²。右 Ki=50 已通过架空和轻载恢复初测，地面长距离仍需复测。
...
- UART3/Serial通用模块仍保留，当前为115200波特率蓝牙链路。速度环调试命令保留`PING`、`INFO`、`GET PID`、`DIST`、停机时的`SET LKP/LKI/LKD/RKP/RKI/RKD value`、`START 100/200/300/400`和`STOP`；每次START会清零编码器累计距离。当前入口切为航向外环测试：`YAW P value`、`YAW D value`、`YAW START speed`、`YAW GET`和`YAW STOP`，航向参数运行中不可修改且只保存在RAM。OLED 显示航向、角速度和差速修正。
